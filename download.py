...
from ao3_backfill import DEFAULT_CACHE_DIR, MINIMUM_DELAY_SECONDS, CredentialsRejected, _Cooldown
...
from calibre_sync import DOWNLOAD_CUSTOM_COLUMNS
from credentials import AO3Credentials, CredentialError, resolve_credentials
from download_client import (
    ACCOUNT_LOCK_CACHE_PATH,
    AO3DownloadClient,
    AnotherRunActive,
    FetchedWorkPage,
    WorkOutcome,
...
from local_metrics import calculate_epub_metrics
from run_log import (
    DEFAULT_LOG_DIR,
    ProgressTracker,
    RunInterrupted,
...
DEFAULT_LINKS_DIR = Path("links")
DEFAULT_OUTPUT_DIR = Path("downloaded")
DEFAULT_DELAY_SECONDS = 10.0
DEFAULT_FAILURE_LOG = DEFAULT_CACHE_DIR / "download-failures.jsonl"
LEGACY_FAILURE_LOG = Path("download_errors.log")
...
def count_legacy_failures(path: Path = LEGACY_FAILURE_LOG) -> int:
    """Count entries in the old plain-URL failure file, which is no longer a skip list."""
...
def enrich_for_calibre(epub_path: Path, metadata: AO3Metadata) -> None:
    ...
    enrich_epub(epub_path, metadata, calibre_columns=DOWNLOAD_CUSTOM_COLUMNS)
    validate_epub_file(epub_path)
    ...
    expects_columns = any(
        getattr(metadata, attribute) is not None for _, _, _, attribute in DOWNLOAD_CUSTOM_COLUMNS
    )
...
def resolve_ao3_credentials() -> AO3Credentials:
...
def run_downloads(
    ...
    *,
    ...
    legacy_failure_log: Path = LEGACY_FAILURE_LOG,
    lock_path: Path = ACCOUNT_LOCK_CACHE_PATH,
    credentials_loader: Callable[[], AO3Credentials] = resolve_ao3_credentials,
    ...
    legacy = count_legacy_failures(legacy_failure_log)
    ...
    if legacy:
        entries.append((legacy_failure_log.name, f"{legacy} old entries, no longer skipped"))
    ...
    with exclusive_ao3_run(lock_path):
...
if __name__ == "__main__":
    ...
    except (CredentialError, CredentialsRejected, AnotherRunActive) as error:
